...
const Sidebar = (props: SidebarProps) => {
  ...
  const filteredItems = items.filter(item => item.heading.toLowerCase().includes(search.toLowerCase()) || item.key.toLowerCase().includes(search.toLowerCase()) )
  return (
    <aside className="w-64 border-r bg-gray-50 p-4">
      <h2 className="text-xl font-bold text-gray-800 px-4 pb-2 tracking-tight text-center mb-4">Components</h2>
      <input
        type="text"
        placeholder="Search..."
        value={search}
      ...
      <ul className="space-y-2 flex flex-col items-center">
        {filteredItems.length > 0 ? (
          filteredItems.map(item => (
            <li key={item.key} className="w-full">
              <button
                ...
                `}
                onClick={() => {setActive(item.key); onItemSelect?.(item)}}
              >
                {item.heading}
              </button>
            </li>
          ...
          <li className="text-gray-400 text-center w-full">No results</li>
        )}
      </ul>
    </aside>
  )
